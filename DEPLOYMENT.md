...
> **ORM**: 全应用使用 **MyBatis 3.0.3**（mybatis-spring-boot-starter）替代 Spring Data JPA。数据库表结构由 `database/init.sql` 定义，启动时不会自动生成/修改表结构。
...
| http://localhost:6379 | Redis（限流令牌桶，无 Web UI） |
...
| `REDIS_HOST` | `redis` | Redis 主机名（Gateway 限流使用） |
...
| `UPLOAD_PATH` | `./uploads` | 文件上传目录（音频/视频/图片），auth / user / poetry / admin 服务共用 |
...
### Q: 服务启动后报错 `Invalid bound statement` 或 `表不存在`
**A:** MyBatis 不会自动建Schema。请确保 `database/init.sql` 已成功执行，并且 `mybatis.configuration.map-underscore-to-camel-case=true` 已在各服务的 `application.yml` 中配置。
```bash
# 检查表是否已创建
docker exec -it poetry-mysql mysql -u root -proot@123 poetry_app -e "SHOW TABLES;"
```
